...
exports.settings = async (req, res) => {
  ...
  parsed.telegram_notify_mode = parsed.telegram_notify_mode || process.env.TELEGRAM_NOTIFY_MODE || 'group';
...
exports.updateTelegram = async (req, res) => {
  const { telegram_bot_token, telegram_chat_id, telegram_enabled, telegram_bot_inbound, telegram_webhook_url, telegram_notify_mode } = req.body;
  ...
  const notifyMode = ['group', 'dm', 'both'].includes(telegram_notify_mode) ? telegram_notify_mode : 'group';

  if (notifyMode !== 'dm') {
    if (!telegram_chat_id || !telegram_chat_id.trim()) {
      req.flash('error', 'Chat ID wajib diisi untuk mode Grup atau Keduanya');
      return res.redirect('/admin/settings');
    }
  ...
  await Setting.upsert({ setting_key: 'telegram_notify_mode', setting_value: notifyMode });
  ...
  process.env.TELEGRAM_NOTIFY_MODE = notifyMode;

  invalidateCache();
  logSetting(req.user, 'update', { bagian: 'telegram', telegram_enabled, telegram_bot_inbound, telegram_notify_mode: notifyMode }, req);
